...
Progress (lessons, quiz bests, exam attempts, flashcard scheduling, notes, resume spot)
is stored in your browser's `localStorage` and syncs **live** across devices via
**Google sign-in (Firebase)**: sign in once per device in **Settings & data** and
every change pushes to Firestore automatically within a second, while a real-time
listener brings changes from your other devices in as they happen — no sync button.

Requires a one-time Firebase project setup: create a project at
console.firebase.google.com, enable **Authentication → Google**, create a
**Firestore** database, add your Pages domain under Auth → Authorized domains,
paste the web-app config into [`firebase-config.js`](firebase-config.js), and set
Firestore rules to:
